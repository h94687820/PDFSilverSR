...
export default function ReaderScreen() {
  ...
  const goToPage = (nextIndex: number) => {
    if (!document) return;
    const next = clamp(nextIndex, 0, document.pages.length - 1);
    setPageIndex(next);
    const now = Date.now();
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background, paddingTop: insets.top }]}>
      <View style={[styles.topbar, { borderBottomColor: colors.border }]}>
        ...
        <Pressable accessibilityLabel="Page management" onPress={() => { setSelectedPage(pageIndex); setManageOpen(true); }} style={[styles.iconButton, { backgroundColor: colors.card, borderColor: colors.border, borderWidth: 1 }]}><Feather name="sidebar" size={18} color={colors.foreground} /></Pressable>
      </View>
      <View style={[styles.toolrow, { borderBottomColor: colors.border }]}>
        ...
        <Pressable accessibilityLabel="Next page" onPress={() => goToPage(pageIndex + 1)} style={styles.tool}><Feather name="chevron-right" size={19} color={pageIndex === document.pages.length - 1 ? colors.border : colors.foreground} /></Pressable>
      </View>
